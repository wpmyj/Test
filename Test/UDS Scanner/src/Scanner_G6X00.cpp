...
bool CScanner_G6X00::preScanPrep()
{
	...
	if(m_bMultiStream)
	{
		...
		static int nTempCount = m_nMultiTotal;  // 多流待扫纸张数
		...
		if (nTempCount > 0)
		{
			...
			m_mat_image = MultiStreamHandle(matMuilt, m_byteMuilt, info);
			nTempCount --;  // 倒序，多流递减
			...
			{
				if (nTempCount == m_nMultiBack)  // 先传正面（低4位），再传背面(高4位)
				{
	...
	{
		Mat matTemp;
		ContrastAndBright(&matTemp, &m_mat_image, (int)info.bright , (int)info.contra);
	...
	long lXDPI = m_fXResolution; //获得图像x轴分辨率
	long lYDPI = m_fYResolution;

	double dFx = (double)info.resol/lXDPI;
	double dFy = (double)info.resol/lYDPI;
...
bool CScanner_G6X00::AdjustParameter()
{
	...
	float fMax = 0;
	GetMaxResolution(m_byteMultiValue, m_fResolu, fMax);
	m_fXResolution = m_fYResolution = fMax;
...
bool CScanner_G6X00::AutoCorrect(Mat src_img , Mat &dst_img)
{
	...
	if(mark)
	{
		cv2DRotationMatrix(center, 270+degree, 1.0, &map_matrix2);  //degree为角度，不是弧度
		int temp = m_width_rotate;
		m_width_rotate = m_height_rotate;
		m_height_rotate = temp;	
...
Rect CScanner_G6X00::RemoveBlack(Mat src_img)
{
	...
	for(i = 0; i < height; i++)
	{
		for(j = 2; j < width-2; j++)
		{
			...
			if((int)tmpMat.at<uchar>(i,j) <= black && 
				(int)tmpMat.at<uchar>(i,j-1) >= white && (int)tmpMat.at<uchar>(i,j+1) <= black)
			{
				rightpoint_j = j;	
			}	
		}
	...
	rightpoint_j = leftpoint_j = 0;  // 必须清零
...
void CScanner_G6X00::GetMaxResolution(const BYTE _multi, float (&_resolution)[8], float& _max)
{
	float fMax = 0;
	BYTE temp = _multi;
	...
	{
		if( (temp >> i) &1 )  // 判断第i位是否为真，即改多流选项是否选中
		{
			if (_resolution[i] > fMax)
			{
				fMax = _resolution[i];
			}	
		}
...
bool CScanner_G6X00::ContrastAndBright(Mat *pdstImage,Mat *psrcImage,int nBrightValue,int nContraValue)
{
...
cv::Mat CScanner_G6X00::MultiStreamHandle(Mat image, BYTE multi, MULTISTREAM_INFO& info)
{
	...
	info.resol = m_fResolu[index];
	info.bright = m_fBright[index];
	info.compre = m_nCompre[index];
	info.compval = m_fCompVal[index];
	info.compqua = m_nCompQua[index];
	info.binari = m_nBinari[index];
	info.thres = m_fThres[index];
	info.removespots = m_fRemovespots[index];
